...
(define-fungible-token NewYorkCoin TOTAL_SUPPLY) ;; 1m tokens

(define-data-var tokenName (string-ascii 32) "NewYorkCoin")
(define-data-var tokenSymbol (string-ascii 10) "NYC")
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		...
		(ft-mint? NewYorkCoin amount recipient)
	)
)
...
(define-public (burn (amount uint) (owner principal))
	(begin
		...
		(ft-burn? NewYorkCoin amount owner)
	)
)
...
(define-private (mint-many-iter (item {amount: uint, recipient: principal}))
	(ft-mint? NewYorkCoin (get amount item) (get recipient item))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(ft-transfer? NewYorkCoin amount sender recipient)
	)
)
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance NewYorkCoin who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply NewYorkCoin))
)
