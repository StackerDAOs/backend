...
(define-fungible-token CityCoins TOTAL_SUPPLY) ;; 1m tokens

(define-data-var tokenName (string-ascii 32) "CityCoins")
(define-data-var tokenSymbol (string-ascii 10) "CITY")
...
(define-private (mint-many-iter (item {amount: uint, recipient: principal}))
	(ft-mint? CityCoins (get amount item) (get recipient item))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(ft-transfer? CityCoins amount sender recipient)
	)
)
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance CityCoins who))
)
