...
(define-constant ERR_PROPOSAL_ALREADY_STARTED (err u3006))
(define-constant ERR_PROPOSAL_INACTIVE (err u3007))
(define-constant ERR_PROPOSAL_NOT_CONCLUDED (err u3008))
(define-constant ERR_NO_VOTES_TO_RETURN (err u3009))
(define-constant ERR_END_BLOCK_HEIGHT_NOT_REACHED (err u3010))
(define-constant ERR_DISABLED (err u3011))
(define-constant ERR_QUORUM_THRESHOLD_NOT_REACHED (err u3012))
...
(define-public (conclude (proposal <proposal-trait>))
  (let
    ...
    (print {event: "conclude", proposal: proposal, totalVotes: totalVotes, quorum: quorum, passed: passed})
